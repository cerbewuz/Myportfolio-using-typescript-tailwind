...
export default function Education({ id = "education" }: EducationProps) {
  ...
  return (
    <section id={id} className="py-24 px-6 max-w-5xl mx-auto border-t border-gray-200 dark:border-gray-800/50">
      ...
      <div ref={containerRef} className="relative space-y-12 md:space-y-16">
        ...
        <motion.div 
          style={{ scaleY }}
          className="absolute left-[7px] md:left-[calc(33.333%-24px)] top-2 bottom-0 w-[2px] bg-gray-600 dark:bg-gray-400 origin-top z-10"
        />
        ...
        {educationData.map((item, i) => (
          ...
          >
            ...
            <div className="md:w-2/3 space-y-2">
              <h3 className="text-base sm:text-xl font-semibold tracking-tight transition-colors group-hover:text-gray-900 dark:group-hover:text-gray-100">{item.degree}</h3>
              <p className="text-sm sm:text-base text-gray-600 dark:text-gray-400 font-medium">{item.institution}</p>
              
              <p className="pt-1 text-sm sm:text-base text-gray-500 dark:text-gray-400 leading-relaxed text-pretty group-hover:text-gray-700 dark:group-hover:text-gray-300 transition-colors">
                {item.description}
              </p>
            </div>
          </motion.div>
