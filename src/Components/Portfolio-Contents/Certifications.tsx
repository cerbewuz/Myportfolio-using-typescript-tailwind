      ...
      <div className="grid grid-cols-2 gap-4 sm:gap-8 lg:grid-cols-3">
        {certificationList.map((cert, i) => (
          <motion.div
            ...
            className="group relative p-3 sm:p-6 md:p-8 rounded-xl md:rounded-2xl bg-gray-50 dark:bg-[#0c0c0c] border border-gray-200/50 dark:border-gray-800/50 hover:border-gray-400 dark:hover:border-gray-600 transition-all duration-300 flex flex-col items-center text-center cursor-pointer"
          >
            {/* Badge Container */}
            <div className="w-12 h-12 sm:w-20 md:w-24 mb-3 sm:mb-6 relative flex items-center justify-center overflow-hidden transition-transform duration-500 group-hover:scale-110">
              <img
                src={cert.badge}
            ...
            <div className="space-y-0.5 sm:space-y-1 flex-grow mb-2 sm:mb-6">
              <h3 className="text-[10px] sm:text-base md:text-lg font-bold tracking-tight leading-tight sm:leading-snug group-hover:text-gray-600 dark:group-hover:text-gray-300 transition-colors px-0.5 sm:px-2">
                {cert.title}
              </h3>
              <p className="text-[8px] sm:text-xs md:text-sm font-medium text-gray-500 dark:text-gray-400">
                {cert.issuer}
              </p>

              <div className="pt-1 sm:pt-4 space-y-0.5 mt-auto">
                <p className="text-[6.5px] sm:text-[10px] font-bold tracking-widest text-gray-400 dark:text-gray-500 uppercase">
                  {cert.date}
                  {cert.expiry && ` • Exp: ${cert.expiry}`}
                </p>
                <p className="text-[6.5px] font-mono text-gray-400/70 truncate max-w-full" title={`ID: ${cert.id}`}>
                  ID: {cert.id}
                </p>
              </div>
            </div>
            ...
            <button
              ...
              className="mt-auto w-full py-1 sm:py-2 px-1.5 sm:px-4 rounded-lg bg-gray-200 dark:bg-gray-800 hover:bg-gray-300 dark:hover:bg-gray-700 text-[8px] sm:text-xs font-bold tracking-tight sm:tracking-widest uppercase transition-colors"
            >
              View Certificate
            </button>
